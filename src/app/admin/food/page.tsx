...
setDeleteCode
  ...
  const {
    ...
    isFetched,
    isFetching,
  } = api.list.foodTip.useQuery({ page, perPage });

  const { mutate: createData, isPending: pendingCreate } =
    api.create.foodTip.useMutation({
      onSuccess: async () => {
  ...
  const { mutate: updateData, isPending: pendingUpdate } =
    api.update.foodTip.useMutation({
      onSuccess: async () => {
        await invalidateList();
      },
      onSettled: () => {
  ...
  const { mutate: deleteData, isPending: pendingDelete } =
    api.delete.foodTip.useMutation({
      onSuccess: async () => {
        await invalidateList();
      },
      onSettled: () => {
...
setDeleteCode(false)
  ...
  const invalidateList = async () => {
    await utils.list.foodTip.invalidate();
  };
  const handleSave = () => {
    createData({
      ...formData,
