...
setDeleteCode
  ...
  const {
    ...
    isFetched,
    isFetching,
  } = api.list.homeTip.useQuery({ page, perPage });

  const { mutate: createData, isPending: pendingCreate } =
    api.create.homeTip.useMutation({
      onSuccess: async () => {
  ...
  const { mutate: updateData, isPending: pendingUpdate } =
    api.update.homeTip.useMutation({
      onSuccess: async () => {
        await invalidateList();
      },
      onSettled: () => {
  ...
  const { mutate: deleteData, isPending: pendingDelete } =
    api.delete.homeTip.useMutation({
      onSuccess: async () => {
        await invalidateList();
      },
      onSettled: () => {
...
setDeleteCode(false)
  ...
  const invalidateList = async () => {
    await utils.list.homeTip.invalidate();
  };
  const handleSave = () => {
    createData({
      ...formData,
