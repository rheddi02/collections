...
setDeleteCode
  ...
  const {
    ...
    isFetched,
    isFetching,
  } = api.list.energyTip.useQuery({ page, perPage });

  const { mutate: createData, isPending: pendingCreate } =
    api.create.energyTip.useMutation({
      onSuccess: async () => {
  ...
  const { mutate: updateData, isPending: pendingUpdate } =
    api.update.energyTip.useMutation({
      onSuccess: async () => {
        await invalidateList();
      },
      onSettled: () => {
  ...
  const { mutate: deleteData, isPending: pendingDelete } =
    api.delete.energyTip.useMutation({
      onSuccess: async () => {
        await invalidateList();
      },
      onSettled: () => {
...
setDeleteCode(false)
  ...
  const invalidateList = async () => {
    await utils.list.energyTip.invalidate();
  };
  const handleSave = () => {
    createData({
      ...formData,
