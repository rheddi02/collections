...
import CustomDialog from "~/app/admin/_components/dialog";
...
setDeleteCode
  ...
  const {
    ...
    isFetched,
    isFetching,
  } = api.list.leisureTip.useQuery({ page, perPage });

  const { mutate: createData, isPending: pendingCreate } =
    api.create.leisureTip.useMutation({
      onSuccess: async () => {
  ...
  const { mutate: updateData, isPending: pendingUpdate } =
    api.update.leisureTip.useMutation({
      onSuccess: async () => {
        await invalidateList();
      },
      onSettled: () => {
  ...
  const { mutate: deleteData, isPending: pendingDelete } =
    api.delete.leisureTip.useMutation({
      onSuccess: async () => {
        await invalidateList();
      },
      onSettled: () => {
...
setDeleteCode(false)
  ...
  const invalidateList = async () => {
    await utils.list.leisureTip.invalidate();
  };
  const handleSave = () => {
    createData({
      ...formData,
