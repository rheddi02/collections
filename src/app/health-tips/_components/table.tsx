...
const Table = ({
  ...
  onEdit: (row: Row<healthOutput>) => void;
  onDelete: (row: Row<healthOutput>) => void;
  loading: boolean
}) => {
  const { data, pageCount, setPage } = useHealthStore((state) => ({
    data: state.data,
    pageCount: state.pageCount,
    setPage: state.setPage,
  }));
  const columns: ColumnDef<healthOutput>[] = [
    {
      accessorKey: "title",
      header: () => {
